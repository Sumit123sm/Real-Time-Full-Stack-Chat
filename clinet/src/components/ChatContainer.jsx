  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className="flex items-center gap-3 mx-4 py-3 border-b border-stone-500">
        <img src={selectedUser.profilePPic || assets.avatar_icon} alt="" className='w-8 rounded-full' />
        <p className='flex-1 text-lg text-white flex items-center gap-2'>{selectedUser.fullName}
        {onlineUsers.includes(selectedUser._id) && <span className='w-2 h-2 rounded-full bg-green-500'></span>}
        </p>
        <img src={assets.arrow_icon} onClick={()=>setSelectedUser(null)} className='md:hidden max-w-7' alt="" />
